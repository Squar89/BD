...
BEGIN
  IF :NEW.ocena IS NOT NULL THEN
    ...
    UPDATE film SET film.srednia_ocen = CAST(film.srednia_ocen AS number(*, 2))
                                      + ((CAST(:NEW.ocena AS number(*, 2)) - CAST(film.srednia_ocen AS number(*, 2))) / CAST(film.liczba_ocen AS number(*, 2)))

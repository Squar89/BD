...
CREATE VIEW lista_do_obejrzenia (pozycja, id_filmu, tytul, rok_produkcji, gatunek, rezyser_imie, rezyser_nazwisko, czas_trwania, srednia_ocen, priorytet, uzytkownik_id) AS
SELECT poz.pozycja, f.id, f.tytul, f.rok_produkcji, g.nazwa, r.imie, r.nazwisko, f.czas_trwania, f.srednia_ocen, pr.opis, poz.uzytkownik_id
...
BEGIN
  IF :NEW.ocena IS NOT NULL THEN
    UPDATE film SET film.liczba_ocen = film.liczba_ocen + 1,
                SET film.srednia_ocen = film.srednia_ocen + ((CAST(:NEW.ocena AS number(*, 2)) - film.srednia_ocen) / CAST(film.liczba_ocen AS number(*, 2)))
